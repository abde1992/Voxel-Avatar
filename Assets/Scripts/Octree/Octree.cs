...
public class Octree : IOctree
{
    ...
    public Octree() : this(1.0f, 0.0f, 1.0f, 0.0f, 1.0f, 0.0f, 20, OctreeNode.NoMinSize) { }
    ...
    public Octree(int up, int left, int down, int right, int front, int back, int maxItems)
        : this( up, left, down, right, front, back, maxItems, OctreeNode.DefaultMinSize)
    {}
    ...
    public void GetLeavePos(List<List<Vector3>> ln) {
        Top.GetLeavePos(ln);
	}
